...
test('ヘルプモーダル: 操作説明とMCP接続が表示される', async ({ page }) => {
  await page.getByRole('button', { name: '使い方・ヘルプ' }).click()
  await expect(page.getByRole('heading', { name: 'local-jam の使い方' })).toBeVisible()
  await expect(page.getByText('範囲選択')).toBeVisible()
