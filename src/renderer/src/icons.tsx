...
// アイコン台帳。拡充＝ここに1エントリ追加するだけ（PartType は 'icon' で固定）。
// 線画スタイル（viewBox 0 0 24 24, stroke=currentColor）。色はテーマ(--jam-text)に追従。
interface IconDef {
  label: string
  body: ReactNode
}
...
export const ICON_CHOICES: { value: string; label: string }[] = ICON_NAMES.map((n) => ({
  value: n,
  label: ICONS[n].label
}))
...
// アイコン1個を描画。親要素いっぱいに収まる（正方形ノード前提）。
